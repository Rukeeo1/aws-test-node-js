...
app.get('/shoes', (req, res) => {
  // Read the JSON file
  fs.readFile('shoes.json', 'utf8', (err, data) => {
    if (err) {
      console.error('Error reading JSON file:', err);
      res.status(500).send('Internal Server Error');
      return;
    }
    const shoesData = JSON.parse(data);
    res.json(shoesData);
  });
});
...
app.post('/shoes', (req, res) => {
  ...
  fs.readFile('shoes.json', 'utf8', (err, data) => {
    ...
    // Generate a new ID for the shoe
    const newId = shoesData.length + 1;
    newShoe.id = newId;

    // Add the new shoe to the array
    shoesData.push(newShoe);

    // Convert the updated array to JSON
    const updatedJsonData = JSON.stringify(shoesData);

    // Write the updated JSON data to the file
    fs.writeFile('shoes.json', updatedJsonData, 'utf8', (err) => {
      if (err) {
        console.error('Error writing JSON file:', err);
        res.status(500).send('Internal Server Error');
        return;
      }
      res.json({ message: 'Shoe added successfully.', shoe: newShoe });
    });
  });
});
